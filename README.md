...
[![Sample Output](https://img.youtube.com/vi/0oORDo1Aq50/maxresdefault.jpg)](https://youtu.be/0oORDo1Aq50)

The clip above was generated by running the pipeline with `VIDEO_OUTPUT_MODE = True`. It shows:
- Detected yard lines and sidelines drawn over the broadcast feed
- A tracked player's bounding box with their real-time speed in mph
- Field coordinates (yards along, yards across) displayed below the bounding box
- A bird's-eye minimap in the corner showing the player's position on the field

See [Configuration](#configuration) for how to enable video output in your own run.
